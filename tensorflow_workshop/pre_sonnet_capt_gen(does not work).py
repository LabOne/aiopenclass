...
class Caption_Generator():
    def __init__(self, dim_in, dim_embed, dim_hidden, batch_size, n_lstm_steps, n_words, init_b=None,from_image=False):
        ...
        var_list.append(self.word_embedding)

        self.embedding_bias = tf.Variable(tf.zeros([dim_embed]), name='embedding_bias')
        var_list.append(self.embedding_bias)
        ...
        var_list.append(self.img_embedding)
        self.img_embedding_bias = tf.Variable(tf.zeros([dim_hidden]), name='img_embedding_bias')
        var_list.append(self.img_embedding_bias)


        # declare the variables to go from an LSTM output to a word encoding output
        self.word_encoding = tf.Variable(tf.random_uniform([dim_hidden, n_words], -0.1, 0.1), name='word_encoding')
        var_list.append(self.word_encoding)
        ...
        var_list.append(self.word_encoding_bias)
    ...
    def build_model(self):
        ...
        #explicitly declare RNN scope
        with tf.variable_scope("RNN") as vs:
            rnn_output,self.rnn_state=rnn.dynamic_rnn(self.lstm,input_embeddings,dtype=tf.float32,time_major=False)
            print ([x.name for x in self.var_list])
            self.var_list+=tf.get_collection(tf.GraphKeys.VARIABLES, scope=vs.name)
            print ([x.name for x in self.var_list])
...
def train(learning_rate=0.001, continue_training=False):
    ...
    saver = tf.train.Saver(var_list=caption_generator.var_list,max_to_keep=100)
    train_op = tf.train.AdamOptimizer(learning_rate).minimize(loss)
    tf.global_variables_initializer().run()
    print ([x.name for x in caption_generator.var_list])
    ...
    for epoch in range(n_epochs):
        ...
        learning_rate *= 0.95
def test(sess,image,generated_words,ixtoword,idx=0,var_list=[]): # Naive greedy search
...
if __name__=='__main__':
    ...
    if sys.argv[1]=='train':
        ...
        print ([x.name for x in caption_generator.var_list])
        test(sess,image,generated_words,ixtoword,1,caption_generator.var_list)